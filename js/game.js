...
function update(){
  ...
  if(gs!==ST.PLAY){ updateParticles(); updateFloaters(); return; }
  time+=1/60;
  ...
  for(let ei=enemies.length-1;ei>=0;ei--){
    ...
    if(e.hit>0)e.hit--;
    // separate from other enemies a touch (avoid full overlap)
    ...
    if(e.hp<=0){
      ...
      player.kills++; enemies.splice(ei,1); continue;
    }
    if(player.iframe<=0){
...
function draw(){
  ...
  if(gs===ST.PLAY||gs===ST.LEVELUP) drawHUD();
